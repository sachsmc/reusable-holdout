...
>>>>>>> 8853ed2bc6b87511379ecd30a1aa0e6449cc953c
output: 
    slidy_presentation:
...
Let $\phi: \mathcal{X} \mapsto \mathbb{R}$ denote the performance evaluation function, e.g., accuracy of predictive model. 

> Note that $\phi$ does not depend on $f$, but rather the algorithm used to generate $f$. 

We are interested in estimating $E_\mathcal{P}(\phi)$, the _generalization error_. 

We can estimate that with the in-sample empirical estimate: $E_S(\phi) = \frac{1}{n}\sum_{i=1}^n \phi(s_i)$. 

However, if the analyst __interacts with $S$__ in the definition of $\phi$, the estimate will be biased (overfit). I.e.,

$$
|E_\mathcal{P}(\phi) - E_S(\phi)|
$$
...
1. Split-sample:
    ...
    - Generate an $f$ using $S_t$ only
    - Estimate is $E_{S_h}(\phi)$
2. Cross-validation:
    - Leave out sample $i$ and estimate $f$ using $S_{-i}$
    - Get single estimate $\phi(s_i)$ for that $f$
...
$$
<<<<<<< HEAD
\max_{x \in \mathcal{X}} \frac{P[\mathcal{A}(S) \in x]}{P[\mathcal{A}(S')]} \leq e^\epsilon
=======
...
2. Else, sample $\xi \sim N(0, 2\tau)$, $\gamma \sim N(0, 4\tau)$, and $\eta \sim N(0, 8\tau)$. 
    1. If $|E_{S_h}(\phi) - E_{S_t}(\phi)| > \hat{T} + \eta$, output $E_{S_h}(\phi) + \xi$ and set $B = B - 1$ and $\hat{T} = T + \gamma$. 
    2. Else, output $E_{S_t}(\phi)$. 
...
- $T$ and $\tau$ control the degree of generalization error: 
    $$
    P\left\{|E_\mathcal{P}(\phi) - E_S(\phi)| > \tau\right\} \leq \beta
    $$
- Dwork et al. recommend choosing so $T + \tau = 0.05$
- Budget $B$ is determined by
    $$
    \frac{\tau^2 n^2}{512 \cdot \log(8/\beta)}
    $$
